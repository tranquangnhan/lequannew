...
function submitForm() {
    ...
    if (valColor == "") {
        ...
        for (let i = 0; i < arr.length; i++) {
            // console.log(arr[i].slice(1).length);
            if (arr[i].charAt(0) != '#') { message.innerHTML = "Định dạng hợp lệ là #000, #fff, ..."; return false; } else { message.innerHTML = ""; }
            if (arr[i].slice(1).length == 3 || arr[i].slice(1).length == 6 ) { message.innerHTML = ""; } else { message.innerHTML = "Định dạng hợp lệ là #000, #fff, ..."; return false; }
        }
